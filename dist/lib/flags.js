import { render, error as printError } from './output.js';
import { ApiError, ExitCode } from './client.js';
export function addCommonFlags(cmd) {
    return cmd
...
export function pickRenderFlags(opts) {
    return {
        json: opts.json,
        csv: opts.csv,
...
export function parseFilter(expr) {
    ...
    if (eqIdx === -1) {
        throw new Error(`bad --filter "${expr}" — expected field=value or field=op:value`);
    }
...
export async function runHandler(fn, flags) {
    try {
        const data = await fn();
...
export function handleError(err, flags) {
